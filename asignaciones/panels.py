...
asignaciones_npo_asignada_un_dia = QuerySet(model=AsignacionNpo, query=asignaciones_npo.filter(estado_asignacion=ASIGNADA, fecha_asignacion__lt=timezone.now()).query)
asignaciones_npo_en_monitoreo_tres_dias = QuerySet(model=AsignacionNpo, query=asignaciones_npo.filter(estado_asignacion=EN_MONITOREO, actualizado__lte=THREEDAYS, conceptos_npo__creado__lte=THREEDAYS).distinct().query)
...
asignaciones_ni_asignada_un_dia = QuerySet(model=AsignacionNi, query=asignaciones_ni.filter(estado_asignacion=ASIGNADA, fecha_asignacion__lt=timezone.now()).query)
asignaciones_ni_en_monitoreo_tres_dias = QuerySet(model=AsignacionNi, query=asignaciones_ni.filter(estado_asignacion=EN_MONITOREO, actualizado__lte=THREEDAYS, conceptos_ni__creado__lte=THREEDAYS).distinct().query)
